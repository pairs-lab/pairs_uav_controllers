...
#include <pairs_uav_controllers/se3_controllerConfig.h>
...
private:
  ...
  typedef pairs_uav_controllers::se3_controllerConfig DrsConfig_t;
  typedef dynamic_reconfigure::Server<DrsConfig_t>  Drs_t;
  boost::shared_ptr<Drs_t>                          drs_;
  void                                              callbackDrs(pairs_uav_controllers::se3_controllerConfig& config, uint32_t level);
  ...
  void SE3Controller(const pairs_msgs::UavState& uav_state, const pairs_msgs::TrackerCommand& tracker_command, const double& dt,
...
bool Se3Controller::initialize(const ros::NodeHandle& nh, std::shared_ptr<pairs_uav_managers::control_manager::CommonHandlers_t> common_handlers,
                               std::shared_ptr<pairs_uav_managers::control_manager::PrivateHandlers_t> private_handlers) {
  ...
  private_handlers->param_loader->addYamlFile(ros::package::getPath("pairs_uav_controllers") + "/config/private/se3_controller.yaml");
  private_handlers->param_loader->addYamlFile(ros::package::getPath("pairs_uav_controllers") + "/config/public/se3_controller.yaml");

  const std::string yaml_namespace = "pairs_uav_controllers/se3_controller/";
  ...
  profiler_ = pairs_lib::Profiler(common_handlers_->parent_nh, "Se3Controller", _profiler_enabled_);
...
void Se3Controller::updateInactive(const pairs_msgs::UavState& uav_state, [[maybe_unused]] const std::optional<pairs_msgs::TrackerCommand>& tracker_command) {
...
Se3Controller::ControlOutput Se3Controller::updateActive(const pairs_msgs::UavState& uav_state, const pairs_msgs::TrackerCommand& tracker_command) {

  pairs_lib::Routine    profiler_routine = profiler_.createRoutine("updateActive");
  pairs_lib::ScopeTimer timer = pairs_lib::ScopeTimer("Se3Controller::updateActive", common_handlers_->scope_timer.logger, common_handlers_->scope_timer.enabled);
...
const pairs_msgs::ControllerStatus Se3Controller::getStatus() {
...
void Se3Controller::switchOdometrySource(const pairs_msgs::UavState& new_uav_state) {
...
const pairs_msgs::DynamicsConstraintsSrvResponse::ConstPtr Se3Controller::setConstraints([
...
void Se3Controller::SE3Controller(const pairs_msgs::UavState& uav_state, const pairs_msgs::TrackerCommand& tracker_command, const double& dt,
                                  const common::CONTROL_OUTPUT& output_modality) {
...
void Se3Controller::positionPassthrough(const pairs_msgs::UavState& uav_state, const pairs_msgs::TrackerCommand& tracker_command) {
...
void Se3Controller::PIDVelocityOutput(const pairs_msgs::UavState& uav_state, const pairs_msgs::TrackerCommand& tracker_command,
...
void Se3Controller::callbackDrs(pairs_uav_controllers::se3_controllerConfig& config, [[maybe_unused]] uint32_t level) {
...
void Se3Controller::timerGains(const ros::TimerEvent& event) {
  ...
  pairs_lib::ScopeTimer timer = pairs_lib::ScopeTimer("Se3Controller::timerGains", common_handlers_->scope_timer.logger, common_handlers_->scope_timer.enabled);
...
double Se3Controller::getHeadingSafely(const pairs_msgs::UavState& uav_state, const pairs_msgs::TrackerCommand& tracker_command) {
