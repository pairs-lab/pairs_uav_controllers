...
bool FailsafeController::initialize(const rclcpp::Node::SharedPtr &node, std::shared_ptr<pairs_uav_managers::control_manager::CommonHandlers_t> common_handlers,
                                    std::shared_ptr<pairs_uav_managers::control_manager::PrivateHandlers_t> private_handlers) {
  ...
  private_handlers->param_loader->addYamlFile(ament_index_cpp::get_package_share_directory("pairs_uav_controllers") + "/config/private/failsafe_controller.yaml");
  private_handlers->param_loader->addYamlFile(ament_index_cpp::get_package_share_directory("pairs_uav_controllers") + "/config/public/failsafe_controller.yaml");
  ...
  sh_hw_api_orientation_ =
      pairs_lib::SubscriberHandler<geometry_msgs::msg::QuaternionStamped>(shopts, "/" + common_handlers->uav_name + "/" + "hw_api/orientation");

  // | ----------- calculate the default hover throttle ----------- |

  hover_throttle_ = pairs_lib::quadratic_throttle_model::forceToThrottle(common_handlers_->throttle_model, _uav_mass_ * common_handlers_->g, *node_);

  // | ------------------------ profiler ------------------------ |

  profiler_ = pairs_lib::Profiler(common_handlers->parent_node, "FailsafeController", _profiler_enabled_);
...
void FailsafeController::updateInactive(const pairs_msgs::msg::UavState                                       &uav_state,
...
FailsafeController::ControlOutput FailsafeController::updateActive(const pairs_msgs::msg::UavState                        &uav_state,
                                                                   [[maybe_unused]] const pairs_msgs::msg::TrackerCommand &tracker_command) {

  pairs_lib::Routine    profiler_routine = profiler_.createRoutine("update");
  pairs_lib::ScopeTimer timer =
      pairs_lib::ScopeTimer(node_, "FailsafeController::update", common_handlers_->scope_timer.logger, common_handlers_->scope_timer.enabled);
  ...
  if (highest_modality.value() == common::VELOCITY_HDG) {
    ...
    pairs_msgs::msg::HwApiVelocityHdgCmd vel_cmd;
  ...
  if (highest_modality.value() == common::VELOCITY_HDG_RATE) {
    ...
    pairs_msgs::msg::HwApiVelocityHdgRateCmd vel_cmd;
  ...
  if (highest_modality.value() == common::ACCELERATION_HDG) {
    ...
    pairs_msgs::msg::HwApiAccelerationHdgCmd acc_cmd;
  ...
  if (highest_modality.value() == common::ACCELERATION_HDG_RATE) {
    ...
    pairs_msgs::msg::HwApiAccelerationHdgRateCmd acc_cmd;
...
const pairs_msgs::msg::ControllerStatus FailsafeController::getStatus() {
...
void FailsafeController::switchOdometrySource([[maybe_unused]] const pairs_msgs::msg::UavState &new_uav_state) {
}
...
const std::shared_ptr<pairs_msgs::srv::DynamicsConstraintsSrv::Response>
FailsafeController::setConstraints([[maybe_unused]] const std::shared_ptr<pairs_msgs::srv::DynamicsConstraintsSrv::Request> &constraints) {
...
double FailsafeController::getHeadingSafely(const geometry_msgs::msg::QuaternionStamped::ConstSharedPtr quaternion) {
  ...
  try {
    return pairs_lib::AttitudeConverter(quaternion->quaternion).getHeading();
  }
  catch (...) {
  }

  try {
    return pairs_lib::AttitudeConverter(quaternion->quaternion).getYaw();
  }
  catch (...) {
  }
