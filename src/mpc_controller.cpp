...
#include <pairs_uav_controllers/mpc_controllerConfig.h>
...
private:
  ...
  typedef pairs_uav_controllers::mpc_controllerConfig DrsConfig_t;
  typedef dynamic_reconfigure::Server<DrsConfig_t>  Drs_t;
  boost::shared_ptr<Drs_t>                          drs_;
  void                                              callbackDrs(pairs_uav_controllers::mpc_controllerConfig &config, uint32_t level);
  DrsConfig_t                                       drs_params_;

  // | ----------------------- controllers ---------------------- |

  void positionPassthrough(const pairs_msgs::UavState &uav_state, const pairs_msgs::TrackerCommand &tracker_command);

  void PIDVelocityOutput(const pairs_msgs::UavState &uav_state, const pairs_msgs::TrackerCommand &tracker_command, const common::CONTROL_OUTPUT &control_output,
                         const double &dt);

  void MPC(const pairs_msgs::UavState &uav_state, const pairs_msgs::TrackerCommand &tracker_command, const double &dt,
...
bool MpcController::initialize(const ros::NodeHandle &nh, std::shared_ptr<pairs_uav_managers::control_manager::CommonHandlers_t> common_handlers,
                               std::shared_ptr<pairs_uav_managers::control_manager::PrivateHandlers_t> private_handlers) {
  ...
  private_handlers->param_loader->addYamlFile(ros::package::getPath("pairs_uav_controllers") + "/config/private/mpc_controller.yaml");
  private_handlers->param_loader->addYamlFile(ros::package::getPath("pairs_uav_controllers") + "/config/public/mpc_controller.yaml");
  private_handlers->param_loader->addYamlFile(ros::package::getPath("pairs_uav_controllers") + "/config/private/" + private_handlers->name_space + ".yaml");
  private_handlers->param_loader->addYamlFile(ros::package::getPath("pairs_uav_controllers") + "/config/public/" + private_handlers->name_space + ".yaml");

  const std::string yaml_namespace = "pairs_uav_controllers/" + private_handlers_->name_space + "/";
  ...
  sh_imu_ = pairs_lib::SubscribeHandler<sensor_msgs::Imu>(shopts, "/" + common_handlers->uav_name + "/hw_api/imu");
  ...
  profiler_ = pairs_lib::Profiler(common_handlers->parent_nh, "MpcController", _profiler_enabled_);
...
bool MpcController::activate(const ControlOutput &last_control_output) {
  ...
  if (_rampup_enabled_ && throttle_last_controller) {
    ...
    double hover_throttle = pairs_lib::quadratic_throttle_model::forceToThrottle(common_handlers_->throttle_model, activation_mass * common_handlers_->g);
...
void MpcController::updateInactive(const pairs_msgs::UavState &uav_state, [[maybe_unused]] const std::optional<pairs_msgs::TrackerCommand> &tracker_command) {
...
MpcController::ControlOutput MpcController::updateActive(const pairs_msgs::UavState &uav_state, const pairs_msgs::TrackerCommand &tracker_command) {

  pairs_lib::Routine    profiler_routine = profiler_.createRoutine("updateActive");
  pairs_lib::ScopeTimer timer = pairs_lib::ScopeTimer("MpcController::updateActive", common_handlers_->scope_timer.logger, common_handlers_->scope_timer.enabled);
...
const pairs_msgs::ControllerStatus MpcController::getStatus() {
...
void MpcController::switchOdometrySource(const pairs_msgs::UavState &new_uav_state) {
...
const pairs_msgs::DynamicsConstraintsSrvResponse::ConstPtr MpcController::setConstraints([
...
void MpcController::MPC(const pairs_msgs::UavState &uav_state, const pairs_msgs::TrackerCommand &tracker_command, const double &dt,
                        const common::CONTROL_OUTPUT &output_modality) {
  ...
  if (rampup_active_) {
    ...
    if (desired_thrust_force >= 0) {
      throttle = pairs_lib::quadratic_throttle_model::forceToThrottle(common_handlers_->throttle_model, desired_thrust_force);
    } else {
...
void MpcController::positionPassthrough(const pairs_msgs::UavState &uav_state, const pairs_msgs::TrackerCommand &tracker_command) {
...
void MpcController::PIDVelocityOutput(const pairs_msgs::UavState &uav_state, const pairs_msgs::TrackerCommand &tracker_command,
...
void MpcController::callbackDrs(pairs_uav_controllers::mpc_controllerConfig &config, [[maybe_unused]] uint32_t level) {
...
void MpcController::timerGains(const ros::TimerEvent &event) {
  ...
  pairs_lib::Routine    profiler_routine = profiler_.createRoutine("timerGains", _gain_filtering_rate_, 1.0, event);
  pairs_lib::ScopeTimer timer = pairs_lib::ScopeTimer("MpcController::timerGains", common_handlers_->scope_timer.logger, common_handlers_->scope_timer.enabled);
...
double MpcController::getHeadingSafely(const pairs_msgs::UavState &uav_state, const pairs_msgs::TrackerCommand &tracker_command) {
